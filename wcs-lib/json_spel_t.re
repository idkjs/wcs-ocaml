...
let rec yojson_of_json_spel = (j: json_spel): Yojson.Basic.t =>
  switch (j) {
  | `Assoc(l) =>
    `Assoc(List.map(x => (fst(x), yojson_of_json_spel(snd(x))), l))
  | `Bool(b) => `Bool(b)
  | `Float(f) => `Float(f)
  | `Int(i) => `Int(i)
...
let rec json_spel_of_yojson = (j: Yojson.Basic.t): json_spel =>
  switch (j) {
  | `Assoc(l) =>
    `Assoc(List.map(x => (fst(x), json_spel_of_yojson(snd(x))), l))
  | `Bool(b) => `Bool(b)
  | `Float(f) => `Float(f)
  | `Int(i) => `Int(i)
...
let to_string = x => Yojson.Basic.to_string(yojson_of_json_spel(x));
