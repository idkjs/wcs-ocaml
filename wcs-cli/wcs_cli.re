...
let wcs_cred: ref(option(string)) = (ref(None): ref(option(string)));
let set_wcs_credential = f => wcs_cred := Some(f);

let command = ref(Cmd_nothing);

let unset_error_recovery = () => Log.error_recovery := false;

let set_debug = () => Log.debug_message := true;
...
let speclist = [
  ...
  (
    "-no-error-recovery",
    Arg.Unit(unset_error_recovery),
    " Do not try to recover in case of error.",
  ),
  ("-debug", Arg.Unit(set_debug), " Print debug messages."),
];
...
let set_command = cmd =>
  ...
  | _ =>
    let msg = Format.sprintf("'%s' is not a %s command", cmd, cmd_name);
...
let main = () => {
  ...
  let wcs_credential =
    try(Wcs_bot.get_credential(wcs_cred^)) {
    | [@implicit_arity] Log.Error(_, msg) =>
      Arg.usage(
        speclist,
        msg
