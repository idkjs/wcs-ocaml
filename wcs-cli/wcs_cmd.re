...
let set_list_page_limit = n => list_page_limit := Some(n);

let list_include_count = ref(None);
let set_list_include_count = b => list_include_count := Some(b);

let list_sort = ref(None);
let set_list_sort = s =>
  list_sort :=
    Some(
      Wcs_j.sort_workspace_criteria_of_string(
        Yojson.Basic.to_string(`String(s)),
      ),
    );

let list_cursor = ref(None);
let set_list_cursor = s => list_cursor := Some(s);

let list_short = ref(false);
let set_list_short = () => list_short := true;

let list_speclist = [
  (
    "-page_limit",
    Arg.Int(set_list_page_limit),
    "n The number of records to return in each page of results.",
  ),
  (
    "-include_count",
    Arg.Bool(set_list_include_count),
    "b Whether to include information about the number of records returned.",
  ),
  (
    "-sort",
    Arg.String(set_list_sort),
...
let list = wcs_cred => {
  ...
  let rsp = Wcs_call.list_workspaces(wcs_cred, req);
  switch (list_short^) {
  | false => Format.printf("%s@.", Wcs_pretty.list_workspaces_response(rsp))
  | true =>
    List.iter(
      ws => {
        let name =
          switch (ws.ws_rsp_name) {
          | Some(n) => n
          | None => ""
          };

        Format.printf("%s %s@.", ws.ws_rsp_workspace_id, name);
      },
...
let create_anon_args = s => create_ws_fnames := create_ws_fnames^ @ [s];

let create_usage = cmd_name =>
  "Usage:\n"
  ++ "  "
  ++ cmd_name
  ++ " create [options] [workspace.json ...]\n"
  ++ "Options:";

let create = wcs_cred =>
  List.iter(
    fname => {
      let ws = Json.read_json_file(Wcs_j.read_workspace, fname);

      let rsp = Wcs_call.create_workspace(wcs_cred, ws);
      let name =
        switch (rsp.crea_rsp_name) {
        | Some(name) => name
        | None => "?"
        };
...
let delete_anon_args = s => delete_ws_ids := delete_ws_ids^ @ [s];

let delete_usage = cmd_name =>
  "Usage:\n"
  ++ "  "
  ++ cmd_name
  ++ " delete [options] [workspace_id ...]\n"
  ++ "Options:";
...
let set_get_export = () => get_export := Some(true);
...
let get_speclist = [
  (
    "-export",
    Arg.Unit(set_get_export),
    " To include all element content in the returned data.",
  ),
];

let get_anon_args = s => get_ws_ids := get_ws_ids^ @ [s];
...
let get = wcs_cred => {
  let workspaces =
    List.fold_left(
      (acc, id) => {
        let req = Wcs.get_workspace_request(~export=?get_export^, id);

        let ws = Wcs_call.get_workspace(wcs_cred, req);
  ...
  switch (workspaces) {
  | [ws] => Format.printf("%s@.", Yojson.Basic.pretty_to_string(ws))
  | workspaces =>
    Format.printf(
      "%s@.",
...
let update_usage = cmd_name =>
  ...
  ++ "  "
  ++ cmd_name
  ++ " update [options] workspace.json workspace_id\n"
  ++ "Options:";

let update = (wcs_cred, cmd_name) =>
  switch (update_ws_id^, update_ws_fname^) {
  | (Some(id), Some(fname)) =>
    let ws = Json.read_json_file(Wcs_j.read_workspace, fname);
  ...
  | _ =>
    let usage =
      Format.sprintf(
        "%s update: workspace file and workspace id required",
        cmd_name,
      );

    Log.error("Wcs_cli", Some(), Arg.usage_string(update_speclist, usage));
  };
...
let set_logs_filter = b => logs_filter := Some(b);

let logs_sort = ref(None);
let set_logs_sort = s =>
  logs_sort :=
    Some(
      Wcs_j.sort_logs_criteria_of_string(
        Yojson.Basic.to_string(`String(s)),
      ),
    );

let logs_page_limit = ref(None);
let set_logs_page_limit = n => logs_page_limit := Some(n);

let logs_cursor = ref(None);
let set_logs_cursor = s => logs_cursor := Some(s);

let logs_speclist = [
  (
    "-filter",
    Arg.String(set_logs_filter),
    "s A cacheable parameter that limits the results to those matching the specified filter.",
  ),
  (
    "-page_limit",
    Arg.Int(set_logs_page_limit),
    "n The number of records to return in each page of results.",
  ),
  (
    "-sort",
    Arg.String(set_logs_sort),
  ...
  (
    "-cursor",
    Arg.String(set_logs_cursor),
    "token A token identifying the last value from the previous page of results.",
  ),
];

let logs_anon_args = s => logs_ws_ids := logs_ws_ids^ @ [s];

let logs_usage = cmd_name =>
  "Usage:\n"
  ++ "  "
  ++ cmd_name
  ++ " logs [options] [workspace_id ...]"
  ++ "\n"
  ++ "Options:";
...
let logs = wcs_cred =>
  List.iter(
    id => {
      ...
      let rsp = Wcs_call.logs(wcs_cred, id, req);
...
let set_try_context = fname =>
  try({
    let ctx = Yojson.Basic.from_file(~fname, fname);
  ...
  | exn =>
    Log.error(
      "Wcs_cli",
      Some(),
      "Unable to read the context file "
      ++ fname
      ++ ": "
...
let try_text = ref("");
let set_try_text = txt => try_text := txt;

let try_node = ref(None);
let set_try_node = node_id => try_node := Some(node_id);

let try_ws_id = ref(None);

let try_speclist = [
  ("-context", Arg.String(set_try_context), "ctx.json The initial context."),
  ("-text", Arg.String(set_try_text), "txt The initial user input."),
  (
    "-node",
    Arg.String(set_try_node),
...
let try_ = (wcs_cred, cmd_name) => {
  let ws_main_id =
    ...
    | Some(id) => id
    | None =>
      let usage = Format.sprintf("%s try: workspace id required", cmd_name);

      Log.error("Wcs_cli", None, Arg.usage_string(update_speclist, usage));
    };
