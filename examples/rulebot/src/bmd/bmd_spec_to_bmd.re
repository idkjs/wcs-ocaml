...
let rec find_fields = (cname, spec) =>
  ...
  | [BC_concept(_), ..._]
  | [BC_enum(_), ..._] => []
  | [[@implicit_arity] BC_field(cname', fname, btype), ...spec] =>
    if (cname' == cname) {
      [(fname, btype), ...find_fields(cname, spec)];
    } else {
      find_fields(cname, spec);
    }
  };
...
let bmd_concept_of_concept = (spec, cname1, cname2) => {
  let fields = find_fields(cname1, spec);
...
let rec bmd_concepts_of_spec = spec =>
  switch (spec) {
  | [] => []
  | [[@implicit_arity] BC_concept(cname1, cname2), ...spec] => [
      bmd_concept_of_concept(spec, cname1, cname2),
      ...bmd_concepts_of_spec(spec),
    ]
  | [[@implicit_arity] BC_enum(cname, enumlist), ...spec] => [
      bmd_concept_of_enum(cname, enumlist),
      ...bmd_concepts_of_spec(spec),
    ]
  | [BC_field(_), ...spec] => bmd_concepts_of_spec(spec)
  };
