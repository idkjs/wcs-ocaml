...
let entity_value_of_bmd = c => {
  let (en, clean_en) = c;
  let syns = [clean_en, "the " ++ clean_en];
  {
    e_val_value: en,
    e_val_metadata: None,
    e_val_synonyms: syns,
...
let bmd_type_of_concept = c => {
  let (_, _, bt) = c.concept_desc;
  bt;
};
let bmd_name_of_concept = c => {
  let (bn, _, _) = c.concept_desc;
  bn;
};
...
let process_bmd_fields = bmd => {
  let fields =
    List.concat(
      List.map(
        x => process_bmd_fields_from_type(bmd_type_of_concept(x).type_desc),
        bmd,
      ),
    );
...
let process_bmd_enums = bmd => {
  let fields =
    List.concat(
      List.map(
        x => process_bmd_enums_from_type(bmd_type_of_concept(x).type_desc),
        bmd,
      ),
    );
...
let process_bmd_entities = bmd => {
  let entities = List.map(bmd_name_of_concept, bmd);
...
let entities_of_bmd = (bmd: bmd_schema) => {
  let entity_of_bmd_entity = {
    ...
    e_def_values:
      entity_values_of_bmd(process_bmd_entities(bmd.schema_desc)),
    e_def_created: None,
    e_def_metadata: None,
    e_def_updated: None,
  ...
  [entity_of_bmd_entity, field_of_bmd_entity, enum_of_bmd_entity];
};
...
let patch_wcs_workspace = (wcs_cred, ws_id, ws_name, bmd) => {
  let ws = Wcs.workspace(ws_name, ~entities=entities_of_bmd(bmd), ());

  Wcs_call.update_workspace(wcs_cred, ws_id, ws);
};

let bmd_find_entity = (entity_def_list, entity_name) =>
  List.find(x => x.e_def_entity == entity_name, entity_def_list);

let bmd_getentity_values = (bmd, entity_name) =>
  try(
    List.map(
      (x: Wcs_j.entity_value) => x.e_val_value,
      bmd_find_entity(entities_of_bmd(bmd), entity_name).e_def_values,
    )
  ) {
  | Not_found => []
  };
