...
let ws_actn = bmd => {
  ...
  let ws_nodes =
    Ws_common.make_top_dialog(((module SD)) =>
      open SD;
      let print_action = {
        let instr = A_print(mk_expr_undefined());
        dialog_node(
          "Print Action",
      ...
      add_node(print_action);

      let emit_action = {
        let instr = A_emit(mk_expr_undefined());
        dialog_node(
          "Emit Action",
      ...
      add_node(emit_action);

      let define_action_filled = {
        let instr =
          [@implicit_arity] A_define("$has_string", mk_expr_undefined());
        dialog_node(
          "Define Action Filled",
          ~conditions=
            "#mk_define && intents[0].confidence > 0.9  && $has_string",
      ...
      add_node(define_action_filled);

      let define_action =
        dialog_node(
          "Define Action",
          ~text=
            "Ok, let's add a define action, what is the name of the variable to be defined?",
          ~conditions="#mk_define && intents[0].confidence > 0.9",
          (),
        );

      add_node(define_action);

      let variable_name = {
        let instr =
          [@implicit_arity] A_define("<? input.text ?>", mk_expr_undefined());
        dialog_node(
          "Variable Name",
          ~parent=define_action,
          ~text=
            "Ok, I'm adding a new definition for variable '<? input.text ?>'.",
          ~context=
            Ctx.set_actn_desc(
              Ctx.set_skip_user_input(`Null, false),
              "actn_desc",
      ...
      let set_action =
        dialog_node(
          ...
          ~text=
            "Ok, let's add a set action, what is the name of the corresponding variable?",
          (),
        );
      ...
      let set_action_cont =
        dialog_node(
          ...
          ~parent=set_action,
          ~text=
            "..and then what field of variable '<? input.text ?>' should be set?",
      ...
      let field_name = {
        let instr =
          [@implicit_arity] A_set("@field", "$variable", mk_expr_undefined());
        dialog_node(
          "Field Name",
          ~parent=set_action_cont,
          ~text=
            "Ok, I'm adding a new set action for the field @field of variable '$variable'.",
      ...
      let set_action_filled =
        dialog_node(
          ...
          ~text=
            "Ok, let's add a set action for '$has_string'. Which field of '$has_string' should be set?",
          ~context=`Assoc([("variable", `String("$has_string"))]),
          ~next_step=(field_name, Goto_user_input),
          (),
        );

      add_node(set_action_filled);
      add_node(set_action);
      add_node(set_action_cont);
      add_node(field_name);
