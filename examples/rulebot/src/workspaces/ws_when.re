...
let when_node = {
  let instr = ("@entity", Some("the @entity"));
  dialog_node(
    "When Clause",
...
let help = es =>
  dialog_node(
    ...
    ~text=
      "The available entities are: " ++ Ws_common.mkEnglishList("and", es),
...
let ws_when = bmd => {
  let es = Bmd_to_wcs_entities.bmd_getentity_values(bmd, "entity");
  workspace(
    "rulebot-when",
    ~description="Dialog to build the when part of a rule",
    ~entities=entities(bmd),
    ~intents,
    ~dialog_nodes=[when_node, prompt, help(es), do_not_understand],
    (),
  );
};
