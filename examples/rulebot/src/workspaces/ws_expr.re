...
type intent_desc = {
  ...
  intent_desc_examples: list(string),
};
...
let intent_desc_desc_of_cnl_binop = binop =>
  ...
  | Op_mod =>
    mk_intent_desc(
      ...
      "mod",
      "modulo expression",
      ["mod", "modulo", "mod expression", "modulo expression"],
    )
  | Op_pow =>
    mk_intent_desc(
      "Pow",
...
let intent_desc_desc_of_cnl_aggregate = aggop =>
  ...
  | A_total =>
    mk_intent_desc(
      ...
      "total",
      "total aggregate",
      ["total", "sum", "sum of", "sum over", "add all"],
    )
  | A_avg =>
    mk_intent_desc(
      "Average",
...
let intent_of_intent_desc = desc =>
  Wcs.intent(desc.intent_desc_name, ~examples=desc.intent_desc_examples, ());

let make_binary_intent = binop => {
  let intent_desc = intent_desc_desc_of_cnl_binop(binop);
  intent_of_intent_desc(intent_desc);
};

let make_unary_intent = unop => {
  let intent_desc = intent_desc_desc_of_cnl_unop(unop);
  intent_of_intent_desc(intent_desc);
};

let make_aggregate_intent = aggop => {
  let intent_desc = intent_desc_desc_of_cnl_aggregate(aggop);
  intent_of_intent_desc(intent_desc);
};
...
let intents =
  Wcs.(
    [
      ...
      intent(
        "mk_integer",
        ~examples=["int", "integer", "integer value", "int value", "numeral"],
        (),
      ),
      Wcs.intent(
        "mk_real",
      ...
      Wcs.intent("mk_number", ~examples=["number", "numeric"], ()),
      Wcs.intent(
        "mk_literal",
        ~examples=[
          "constant",
      ...
      intent(
        "mk_aggregate",
        ~examples=["aggregate", "reduce", "reduction"],
        (),
      ),
      intent(
        "mk_string",
      ...
      intent(
        "mk_enum",
        ~examples=["enum", "enumeration", "enumeration value", "alternative"],
        (),
      ),
      intent(
        "mk_boolean",
...
let ws_expr = bmd => {
  let entities =
    [Ws_common.entity_boolean, Wcs.sys_number]
    @ Bmd_to_wcs_entities.entities_of_bmd(bmd);

  let integer_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      open SD;
      let _expr_integer_with_value = {
        let expr = mk_int_as_string_f("@sys-number");
        add_dialog_node(
          "Integer with value",
  ...
  let real_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      ...
      let _expr_real_with_value = {
        let expr = mk_float_as_string_f("@sys-number");
        add_dialog_node(
          "Real with value",
  ...
  let number_subdialogs = [integer_dialog, real_dialog];
  let number_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      open SD;
      let expr_number =
  ...
  let boolean_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      ...
      let _expr_literal_boolean_literal_with_value = {
        let expr = mk_boolean_as_string_f("@boolean");
        add_dialog_node(
          "Boolean literal with value",
          ~conditions="#mk_boolean && entities['boolean']",
          ~context=
            json_replace(
              "L_boolean_as_string",
              "L_boolean", /* XXX Hack XXX */
              Ctx.set_expr(`Null, "expr", expr),
            ),
          (),
        );
  ...
  let string_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      ...
      let _expr_string_with_content = {
        let expr = mk_string_f("$has_string");
        add_dialog_node(
          "String with content",
      ...
      let _expr_string_content = {
        let expr = mk_string_f("<? input.text ?>");
        add_dialog_node(
          "String content",
          ~parent=expr_string,
  ...
  let enum_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      ...
      add_usage(
        "Enum",
        "which enumeration value",
        expr_enum_missing,
        Ws_common.bmd_enumerations(bmd),
      );
    );
  ...
  let property_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      open SD;
      let expr_prop = {
        let expr = mk_get_f(mk_expr_undefined(), "@field");
        dialog_node(
          "Property",
      ...
      add_node(expr_prop_fillin);

      add_usage(
        "Prop",
        "which field",
        expr_prop_missing,
        Ws_common.bmd_fields(bmd),
      );
    );
  let aggregate_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      open SD;
      let expr_agg_missing_both =
        dialog_node(
          "Aggregate - missing both",
          ~conditions="#mk_aggregate",
          ~text=
            "Ok, we are creating an aggregate operation.  What aggregation operator should we use?",
          (),
        );
      add_node(expr_agg_missing_both);

      let mk_agg_nodes = op => {
        let op_desc = intent_desc_desc_of_cnl_aggregate(op);
        let agg_dname = "Aggregate _" ++ op_desc.intent_desc_node_name ++ "_ ";

        let expr_agg_full = {
          let expr = mk_aggregate_f(mk_expr_undefined(), op, "@field");
          dialog_node(
            agg_dname ++ "full",
            ~conditions=
              "#" ++ op_desc.intent_desc_name ++ " && entities['field']",
            ~context=Ctx.set_expr(`Null, "expr", expr),
            (),
          );
        };
        ...
        let expr_agg_missing_field =
          dialog_node(
            agg_dname ++ "_ - missing field",
            ~conditions="#" ++ op_desc.intent_desc_name,
            ~text=
              "Ok, we are creating a "
              ++ op_desc.intent_desc_output
              ++ " operation.  What field are we looking at?",
            (),
          );
        ...
        let expr_agg_missing_both_op_and_field = {
          let expr = mk_aggregate_f(mk_expr_undefined(), op, "@field");
          dialog_node(
            agg_dname ++ "- missing both - op and field",
        ...
        let expr_agg_missing_both_op_field = {
          let expr = mk_aggregate_f(mk_expr_undefined(), op, "@field");
          dialog_node(
            agg_dname ++ "- missing both - op - field",
        ...
        add_usage(
          agg_dname ++ "both field",
          "which field",
          expr_agg_missing_both_op,
          Ws_common.bmd_fields(bmd),
        );
      };

      List.iter(mk_agg_nodes, aggregate_op_list);

      add_usage(
        "Aggregate",
        "wich aggregate operation",
        expr_agg_missing_both,
        List.map(
          aggop =>
            intent_desc_desc_of_cnl_aggregate(aggop).intent_desc_output,
          aggregate_op_list,
        ),
      );
    );
  let binary_operators_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      open SD;
      let expr_binary = binop => {
        let intent_desc = intent_desc_desc_of_cnl_binop(binop);
        let expr = mk_binop_f(binop);
        dialog_node(
          intent_desc.intent_desc_node_name,
      ...
      List.iter(b => add_node(expr_binary(b)), binary_op_list);
    );
  let binary_operator_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      open SD;
      ...
      add_usage(
        "Binary operator",
        "what type of binary operator",
        expr_binary_operator,
        List.map(
          binop => intent_desc_desc_of_cnl_binop(binop).intent_desc_output,
          binary_op_list,
        ),
      );
    );
  let unary_operators_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      open SD;
      let expr_unary = unop => {
        let intent_desc = intent_desc_desc_of_cnl_unop(unop);
        let expr = mk_unop_f(unop);
        dialog_node(
          intent_desc.intent_desc_node_name,
          ~conditions="#" ++ intent_desc.intent_desc_name,
          ~text="Alright, let's build an " ++ intent_desc.intent_desc_output,
      ...
      List.iter(b => add_node(expr_unary(b)), unary_op_list);
    );
  let unary_operator_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      open SD;
      ...
      add_usage(
        "Unary operator",
        "what type of unary operator",
        expr_unary_operator,
        List.map(
          unop => intent_desc_desc_of_cnl_unop(unop).intent_desc_output,
          unary_op_list,
        ),
      );
    );
  ...
  let new_dialog =
    Ws_common.make_child_dialog(((module SD)) =>
      ...
      let expr_new_with_entity = (ename, fnames) => {
        let expr = mk_new_event_for_concept_f(ename, fnames);
        dialog_node(
          "new_with_" ++ ename,
          ~conditions=
            "#mk_new && entities['entity']?.contains('" ++ ename ++ "')",
          ~context=Ctx.set_expr(`Null, "expr", expr),
          (),
        );
      };
      ...
      let expr_news = (ename, fnames) => (
        expr_new_with_entity(ename, fnames),
        expr_new_for_entity(ename, fnames),
      );

      let add_expr_new_for_bmd_concept = concept => {
        let (ename, _, ctype) = concept.concept_desc;
        switch (ctype.type_desc) {
        | BT_rec(rec_desc) => [expr_news(ename, List.map(fst, rec_desc))]
        | _ => []
        };
      };

      let add_expr_new_for_bmd = {
        let concepts = bmd.schema_desc;
        List.concat(List.map(add_expr_new_for_bmd_concept, concepts));
      };

      let (top_new_nodes, child_new_nodes) =
        List.split(add_expr_new_for_bmd);
      add_nodes(top_new_nodes);
      add_node(expr_new);
  ...
  let ws_nodes =
    Ws_common.make_top_dialog(((module SD)) =>
      open SD;
