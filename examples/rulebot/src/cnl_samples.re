...
let empty_init = () => {
  let evnt = mk_evnt_undefined();
  let cond = mk_cond_undefined();
  let actns = mk_actns_undefined();
  let r0 = mk_rule_f(evnt, cond, actns);
  index_rule(r0);
};
...
let define_init = (vname): cnl_actn_desc =>
  [@implicit_arity] A_define(vname, mk_expr_undefined());

let set_init = (vname, fname): cnl_actn_desc =>
  [@implicit_arity] A_set(vname, fname, mk_expr_undefined());
      ...
      [
        (
          ...
          mk_concat_list_f([
            ...
            mk_string_f(
              " is risky and below the average balance for risky accounts.",
            ),
          ]),
        ),
...
let cond1 =
  C_condition(
    mk_lt_f(
      mk_get_var_f("balance", "the account"),
      mk_expr_f(E_var("Average Risky Account Balance")),
    ),
  );
let then1 = {
  list_elems: [
    mk_print_f(
      mk_concat_f(
        mk_string_f("aggregate "),
        mk_var_f("Average Risky Account Balance"),
      ),
    ),
    mk_print_f(
      mk_concat_f(
  ...
  list_closed: [@implicit_arity] N_filled(None, ()),
};
  ...
  mk_define_f(
    ...
    mk_div_f(
      mk_plus_f(
        ...
        mk_get_f(
          mk_get_f(mk_this_f("airplane"), "engine"),
          "pressure ratio",
        ),
      ),
      mk_int_f(2),
...
let setdesc22 =
  mk_set_desc_f(
    ...
    "the airplane",
    mk_var_f("pressureAverage"),
  );
let set21 =
  mk_set_f("average engine rpm", "the airplane", mk_var_f("rpmAverage"));
let set22 =
  mk_set_f(
    "average engine pressure ratio",
    "the airplane",
...
let rule2 =
  mk_rule_init(
    ...
    C_no_condition,
    {
      list_elems: [define21, define22, set21, set22],
      list_closed: [@implicit_arity] N_filled(None, ()),
    },
  );
