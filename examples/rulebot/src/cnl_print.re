...
let cnl_print_node = (fpart, kind, ff, part) =>
  switch (part) {
  | N_undefined(i) => fprintf(ff, "[UNDEFINED %a]", cnl_print_id(kind), i)
  | [@implicit_arity] N_filled(i, x) =>
    fprintf(ff, "[%a%a]", cnl_print_id(kind), i, fpart, x)
  | [@implicit_arity] N_rejected(i, x) =>
    fprintf(ff, "[REJECTED %a%a]", cnl_print_id(kind), i, fpart, x)
  | N_accepted(x) => fprintf(ff, "%a", fpart, x)
  };
...
and cnl_print_expr_desc = (prec, ff, ed) => {
  let printer = (ff, x) =>
    switch (x) {
    | E_lit(l) => cnl_print_literal(prec, ff, l)
    | E_var(n) => cnl_print_variable(ff, n)
    | [@implicit_arity] E_get(e, fname) =>
      fprintf(ff, "the %s of %a", fname, cnl_print_expr(prec), e)
    | [@implicit_arity] E_agg(agg, e, fname) =>
      fprintf(
        ff,
        "the %s %s of all %a",
    ...
    | [@implicit_arity] E_unop(op, e) =>
      fprintf(ff, "%s %a", string_of_cnl_unop(op), cnl_print_expr(prec), e)
    | [@implicit_arity] E_binop(op, e1, e2) =>
      let new_prec = cnl_binop_precedence(op);
      if (new_prec < prec) {
        fprintf(
          ff,
          "(%a %s %a)",
          cnl_print_expr(new_prec),
          e1,
    ...
    | E_error(j) => fprintf(ff, "[ERROR]")
    | E_this(s) => fprintf(ff, "this %s event", s)
    | [@implicit_arity] E_new(c, sl) =>
...
and cnl_print_setters = (ff, sl) =>
  ...
  | [s, ...sl'] =>
    fprintf(ff, "%a,@ %a", cnl_print_setter, s, cnl_print_setters, sl')
  };
...
let cnl_print_evnt = (ff, evnt) => {
  let printer = (ff, w) => {
    let (ename, ovname) = w;
    switch (ovname) {
    | None => fprintf(ff, "a %s occurs", ename)
    | Some(vname) => fprintf(ff, "a %s occurs, called '%s'", ename, vname)
    };
  };
  ...
  fprintf(
    ff,
    "@[<hv 2>when %a@;<0 -2>@]",
    cnl_print_node(printer, K_evnt),
    evnt.evnt_node,
  );
};

let cnl_print_cond = (ff, cond) =>
  switch (cond.cond_node) {
  | N_undefined(_) =>
    let printer = (ff, i) => ();
    fprintf(
      ff,
      "@[<hv 2>if %a@;<0 -2>@]",
      cnl_print_node(printer, K_cond),
      cond.cond_node,
    );
  | _ =>
    let printer = (ff, i) =>
      switch (i) {
      | C_condition(i) =>
        fprintf(ff, "@[<hv 2>if %a@;<0 -2>@]", cnl_print_expr(0), i)
      | C_no_condition => ()
      };
...
let cnl_print_actn = (ff, a) => {
  let printer = (ff, ad) =>
    switch (ad) {
    | A_print(e) => fprintf(ff, "print %a", cnl_print_expr(0), e)
    | A_emit(e) => fprintf(ff, "emit %a", cnl_print_expr(0), e)
    | [@implicit_arity] A_define(vname, e) =>
      fprintf(ff, "define '%s' as@ %a", vname, cnl_print_expr(0), e)
    | [@implicit_arity] A_set(fname, vname, e) =>
      fprintf(
        ff,
        "set the %s of '%s' to %a",
        fname,
...
let rec cnl_print_actns_desc = (ff, al) =>
  ...
  | [a, ...al'] =>
    fprintf(ff, "%a;@ %a", cnl_print_actn, a, cnl_print_actns_desc, al')
  };
...
let cnl_print_actns = (ff, actns) => {
  ...
  fprintf(
    ff,
    "@[<hv 2>then %a@;<0 -2>@]",
    cnl_print_node(printer, K_actns),
    actns.actns_node,
  );
};
